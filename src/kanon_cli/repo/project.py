...
class DeleteWorktreeError(RepoError):
    ...
    def __init__(self, *args, aggregate_errors: List[Exception] = None, **kwargs) -> None:
...
# Maximum sleep time allowed during retries. Configurable via environment variable.
MAXIMUM_RETRY_SLEEP_SEC = float(os.environ.get("KANON_MAX_RETRY_SLEEP_SEC", "3600.0"))
# +-10% random jitter is added to each Fetches retry sleep duration. Configurable via env var.
RETRY_JITTER_PERCENT = float(os.environ.get("KANON_RETRY_JITTER_PERCENT", "0.1"))
...
class ReviewableBranch:
    ...
    def commits(self):
        if self._commit_cache is None:
            ...
            try:
                self._commit_cache = self.project.bare_git.rev_list(*args, log_as_error=self.base_exists)
            except GitError:
    ...
    def unabbrev_commits(self):
        r = dict()
        for commit in self.project.bare_git.rev_list(not_rev(self.base), R_HEADS + self.name, "--"):
            r[commit[0:8]] = commit
        return r

    @property
    def date(self):
        return self.project.bare_git.log("--pretty=format:%cd", "-n", "1", R_HEADS + self.name, "--")
...
class _LinkFile:
    ...
    def __init__(self, git_worktree, src, topdir, dest, exclude=None):
        ...
        self.dest = dest
        if exclude:
            self.exclude = frozenset(name for name in (n.strip() for n in exclude.split(",")) if name)
        else:
        ...
        if not glob.has_magic(src):
            ...
            if self.exclude:
                if not os.path.isdir(src):
                    raise ManifestInvalidPathError(f"exclude attribute requires src to be a directory: {self.src}")
            ...
            if self.exclude:
                raise ManifestInvalidPathError(
                    f"exclude attribute cannot be combined with glob patterns in src: {self.src}"
                )
            dest = _SafeExpandPath(self.topdir, self.dest)
...
class Project:
    ...
    def UpdatePaths(self, relpath, worktree, gitdir, objdir):
        ...
        self.config = GitConfig.ForRepository(gitdir=self.gitdir, defaults=self.manifest.globalConfig)
    ...
    def IsDirty(self, consider_untracked=True):
        ...
        self.work_git.update_index("-q", "--unmerged", "--ignore-missing", "--refresh")
    ...
    def PrintWorkTreeStatus(self, output_redir=None, quiet=False, local=False):
        ...
        for p in sorted(set(paths)):
            ...
            if i and i.src_path:
                line = f" {i_status}{f_status}\t{i.src_path} => {p} ({i.level}%)"
            else:
    ...
    def PrintWorkTreeDiff(self, absolute_paths=False, output_redir=None, local=False):
        """Prints the status of the repository to stdout."""
    ...
    def UploadForReview(
        ...
        for label in labels:
            if not re.match(r"^.+[+-][0-9]+$", label):
                raise UploadError(
                    f'invalid label syntax "{label}": labels use forms like CodeReview+1 or Verified-1',
                    project=self.name,
                )
        ...
        if patchset_description:
            opts += [f"m={self._encode_patchset_description(patchset_description)}"]
        if opts:
        ...
        if not dryrun:
            ...
            self.bare_git.UpdateRef(R_PUB + branch.name, R_HEADS + branch.name, message=msg)
    ...
    def Sync_NetworkHalf(
        ...
        if archive and not isinstance(self, MetaProject):
            if self.remote.url.startswith(("http://", "https://")):
                msg_template = "%s: Cannot fetch archives from http/https remotes."
                msg_args = self.name
                msg = msg_template % msg_args
                logger.error(msg_template, msg_args)
                return SyncNetworkHalfResult(False, SyncNetworkHalfError(msg, project=self.name))
        ...
        if self.UseAlternates:
            ...
            if not os.path.exists(gitdir_alt):
                ...
                _lwrite(
                    gitdir_alt,
                    os.path.join(os.path.relpath(self.objdir, gitdir_objects), "objects") + "\n",
                )
        ...
        if clone_bundle and alt_dir is None and self._ApplyCloneBundle(initial=is_new, quiet=quiet, verbose=verbose):
            is_new = False
        ...
        if not (optimized_fetch and IsId(self.revisionExpr) and self._CheckForImmutableRevision()):
            remote_fetched = True
            try:
        ...
        if dissociate:
            ...
            if os.path.exists(alternates_file):
                ...
                if p.Wait() != 0:
                    return SyncNetworkHalfResult(
                        remote_fetched,
                        GitError("Unable to repack alternates", project=self.name),
                    )
        ...
        if self.worktree:
            ...
            platform_utils.remove(os.path.join(self.gitdir, "FETCH_HEAD"), missing_ok=True)
    ...
    def GetCommitRevisionId(self):
        ...
        try:
            ...
            raise ManifestInvalidRevisionError(f"revision {self.revisionExpr} in {self.name} not found")
    ...
    def Sync_LocalHalf(
        ...
        if self.relpath == ".":
            ...
            if bad_paths:
                fail(
                    LocalSyncFail(
                        "Refusing to checkout project that writes to protected paths: %s" % (", ".join(bad_paths),),
                        project=self.name,
                    )
                )
                return
        ...
        if branch is None or syncbuf.detach_head:
            ...
            rebase_in_progress = self.IsRebaseInProgress() or self.IsCherryPickInProgress()
        ...
        if not branch.LocalMerge:
            ...
            syncbuf.info(self, "leaving %s; does not track upstream", branch.name)
            try:
        ...
        try:
            self.work_git.merge_base("--is-ancestor", HEAD, revid, log_as_error=False)
                ...
                if upstream_gain and not force_rebase:
                    ...
                    fail(
                        LocalSyncFail(
                            ...
                            "with the --rebase option to force a rebase." % (branch.name, len(upstream_gain)),
                            project=self.name,
                        )
                    )
                    return
    ...
    def AddLinkFile(self, src, dest, topdir, exclude=None):
        ...
        """
        self.linkfiles.append(_LinkFile(self.worktree, src, topdir, dest, exclude=exclude))
    ...
    def DownloadPatchSet(self, change_id, patch_id):
        ...
        cmd.append("refs/changes/%2.2d/%d/%d" % (change_id % 100, change_id, patch_id))
        GitCommand(self, cmd, bare=True, verify_command=True).Wait()
    ...
    def DeleteWorktree(self, verbose=False, force=False):
        ...
        if self.IsDirty():
            if force:
                logger.warning(
                    "warning: %s: Removing dirty project: uncommitted changes lost.",
                    self.RelPath(local=False),
                )
            else:
                msg = "error: %s: Cannot remove project: uncommitted changes are present.\n" % self.RelPath(local=False)
                logger.error(msg)
        ...
        if self.use_git_worktrees:
            ...
            for name in os.listdir(configs):
                ...
                fullpath = os.path.realpath(os.path.join(configs, name, relpath))
                if fullpath == needle:
        ...
        try:
            ...
            if e.errno != errno.ENOENT:
                ...
                logger.error(
                    "error: %s: Failed to delete obsolete checkout; remove manually, then run `repo sync -l`.",
                    self.RelPath(local=False),
                )
            ...
            for f in files:
                ...
                try:
                    ...
                    if e.errno != errno.ENOENT:
                        ...
                        failed = True
                        errors.append(e)
            dirs[:] = [d for d in dirs if not os.path.lexists(os.path.join(root, d, ".git"))]
            dirs_to_remove += [os.path.join(root, d) for d in dirs if os.path.join(root, d) not in dirs_to_remove]
            ...
            if platform_utils.islink(d):
                ...
                try:
                    ...
                    if e.errno != errno.ENOENT:
                        ...
                        errors.append(e)
        if failed:
            rename_path = f"{self.worktree}_repo_to_be_deleted_{int(time.time())}"
            try:
                platform_utils.rename(self.worktree, rename_path)
                logger.warning(
                    "warning: renamed %s to %s. You can delete it, but you might need elevated permissions (e.g. root)",
                    self.worktree,
                    rename_path,
                )
    ...
    def StartBranch(self, name, branch_merge="", revision=None):
        ...
        if R_HEADS + name in all_refs:
            GitCommand(self, ["checkout", "-q", name, "--"], verify_command=True).Wait()
            return True
    ...
    def PruneHeads(self):
        ...
        if cb is not None and not self._revlist(HEAD + "..." + rev) and not self.IsDirty(consider_untracked=False):
        ...
        if kill:
            ...
            try:
                ...
                b = ["branch", "-d"]
                b.extend(kill)
                b = GitCommand(self, b, bare=True, capture_stdout=True, capture_stderr=True)
                b.Wait()
            finally:
                if IsId(old):
    ...
    def _GetSubmodules(self):
        ...
        def get_submodules(gitdir, rev):
            ...
            submodules = []
            for sub_path, sub_url, sub_shallow in zip(sub_paths, sub_urls, sub_shallows):
                try:
    ...
    def _RemoteFetch(
        ...
        if current_branch_only:
            ...
            if is_sha1 or tag_name is not None:
                if self._CheckForImmutableRevision():
                    if verbose:
                        print("Skipped fetching project %s (already have persistent ref)" % self.name)
                    return True
        ...
        if not current_branch_only:
            # Fetch whole repo.
            spec.append(str(("+refs/heads/*:") + remote.ToLocal("refs/heads/*")))
        ...
        for try_n in range(retry_fetches):
            ...
            elif gitcmd.stdout and "error:" in gitcmd.stdout and "HTTP 429" in gitcmd.stdout:
            ...
            # will bring the repository into a good state.
                ...
                "could not parse commit" in gitcmd.stdout or "unable to parse commit" in gitcmd.stdout
            ):
            ...
            # that and now has refs/heads/upstream/foo.
                ...
                gitcmd.stdout and "error:" in gitcmd.stdout and "git remote prune" in gitcmd.stdout and not prune_tried
            ):
                prune_tried = True
                prunecmd = GitCommand(
                    self,
                ...
                continue
            elif ret == 128 and gitcmd.stdout and "fatal: could not read Username" in gitcmd.stdout:
            ...
            if try_n < retry_fetches - 1:
                print(
                    "%s: sleeping %s seconds before retrying" % (self.name, retry_cur_sleep),
                    file=output_redir,
                )
                # Exponential backoff between fetch retries. The sleep duration and
                # its upper bound are configurable via KANON_MAX_RETRY_SLEEP_SEC and
                # KANON_RETRY_JITTER_PERCENT environment variables.
                time.sleep(retry_cur_sleep)
                retry_cur_sleep = min(retry_exp_factor * retry_cur_sleep, MAXIMUM_RETRY_SLEEP_SEC)
                retry_cur_sleep *= 1 - random.uniform(-RETRY_JITTER_PERCENT, RETRY_JITTER_PERCENT)
    ...
    def _ApplyCloneBundle(self, initial=False, quiet=False, verbose=False):
        ...
        if not exist_dst:
            exist_dst = self._FetchBundle(bundle_url, bundle_tmp, bundle_dst, quiet, verbose)
        if not exist_dst:
            return False
    ...
    def _Checkout(self, rev, force_checkout=False, quiet=False):
        ...
        if GitCommand(self, cmd).Wait() != 0:
            if self._allrefs:
                raise GitError(f"{self.name} checkout {rev} ", project=self.name)
    ...
    def _CherryPick(self, rev, ffonly=False, record_origin=False):
        ...
        if GitCommand(self, cmd).Wait() != 0:
            if self._allrefs:
                raise GitError(f"{self.name} cherry-pick {rev} ", project=self.name)
    ...
    def _ResetHard(self, rev, quiet=True):
        ...
        cmd.append(rev)
        if GitCommand(self, cmd).Wait() != 0:
            raise GitError(f"{self.name} reset --hard {rev} ", project=self.name)
        ...
        try:
            ...
            if self.objdir != self.gitdir:
                ...
                try:
                    ...
                    if force_sync:
                        logger.error("Retrying clone after deleting %s", self.gitdir)
                        try:
                            platform_utils.rmtree(os.path.realpath(self.gitdir))
                            if self.worktree and os.path.exists(os.path.realpath(self.worktree)):
                ...
                if ref_dir or mirror_git:
                    ...
                    if ref_dir:
                        if not os.path.isabs(ref_dir):
                            ...
                            # database.
                            ref_dir = os.path.relpath(ref_dir, os.path.join(self.objdir, "objects"))
                        _lwrite(
                            os.path.join(self.objdir, "objects/info/alternates"),
    ...
    def _InitHooks(self, quiet=False):
        ...
        for stock_hook in _ProjectHooks():
            ...
            if name in ("commit-msg",) and not self.remote.review and self is not self.manifest.manifestProject:
            ...
            if os.path.exists(dst):
                ...
                continue
            try:
                platform_utils.symlink(os.path.relpath(stock_hook, os.path.dirname(dst)), dst)
            except OSError as e:
                if e.errno == errno.EPERM:
                    try:
                        os.link(stock_hook, dst)
                    except OSError:
                        raise GitError(self._get_symlink_error_message(), project=self.name)
                else:
                    raise
        ...
        for name in set(to_copy).union(to_symlink):
            try:
                ...
                if name in to_symlink:
                    platform_utils.symlink(os.path.relpath(src, os.path.dirname(dst)), dst)
    ...
    def _InitWorkTree(self, force_sync=False, submodules=False):
        ...
        if self.use_git_worktrees:
            ...
            if init_dotgit:
                _lwrite(os.path.join(self.gitdir, HEAD), f"{self.GetRevisionId()}\n")
    ...
    def _MigrateOldWorkTreeGitDir(cls, dotgit, project=None):
        ...
        if not os.path.islink(os.path.join(dotgit, "refs")):
            raise GitError(f"{dotgit}: unsupported checkout state", project=project)
    ...
    class _GitGetByExec:
        ...
        def __getattr__(self, name):
            ...
            def runner(*args, log_as_error=True, **kwargs):
                ...
                for k in kwargs:
                    raise TypeError(f"{name}() got an unexpected keyword argument {k!r}")
...
class _Failure:
    ...
    def Print(self, syncbuf):
        syncbuf.out.fail("error: %s/: %s", self.project.RelPath(local=False), str(self.why))
        syncbuf.out.nl()
...
class ManifestProject(MetaProject):
    ...
    def Sync(
        ...
        if is_new:
            ...
            if verbose:
                print(
                    "Downloading manifest from %s" % (GitConfig.ForUser().UrlInsteadOf(manifest_url),),
                    file=sys.stderr,
                )
            ...
            if reference:
                ...
                if not os.path.exists(mirrored_manifest_git):
                    mirrored_manifest_git = os.path.join(reference, ".repo/manifests.git")
        ...
        if worktree:
            if mirror:
                ...
                return False
            if submodules:
                logger.error("fatal: --submodules and --worktree are incompatible")
                return False
        ...
        if archive:
            if is_new:
                ...
                logger.error("fatal: --archive is only supported when initializing a new workspace.")
                logger.error("Either delete the .repo folder in this workspace, or initialize in another location.")
                return False
        ...
        if mirror:
            if is_new:
                ...
                logger.error("fatal: --mirror is only supported when initializing a new workspace.")
                logger.error("Either delete the .repo folder in this workspace, or initialize in another location.")
                return False

        if partial_clone is not None:
            if mirror:
                logger.error("fatal: --mirror and --partial-clone are mutually exclusive")
                return False
        ...
        if not standalone_manifest:
            ...
            if is_new or self.CurrentBranch is None:
                try:
                    ...
                    msg = str(e)
                    logger.error("fatal: cannot create default in manifest %s", msg)
                    return False
    ...
    def ConfigureCloneFilterForDepth(self, clone_filter_for_depth):
        ...
        """
        self.config.SetString("repo.clonefilterfordepth", clone_filter_for_depth)
