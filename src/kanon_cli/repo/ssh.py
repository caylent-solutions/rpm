...
_SSH_MASTER_TIMEOUT_SEC = int(os.environ.get("KANON_SSH_MASTER_TIMEOUT_SEC", "10"))
...
class ProxyManager:
    ...
    def _open_unlocked(self, host, port=None):
        ...
        # Busy-poll until the SSH master process exits (indicating failure) or
        # the timeout expires (indicating the master is running successfully).
        # No sleep is used -- readiness is detected by polling the process state.
        deadline = time.monotonic() + _SSH_MASTER_TIMEOUT_SEC
        while time.monotonic() < deadline:
            if p.poll() is not None:
                # Process already exited -- SSH master failed to start.
                return False
