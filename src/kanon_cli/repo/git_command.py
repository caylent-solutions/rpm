...
def GetEventTargetPath():
    ...
    if retval == 0:
        ...
        logger.error(
            "repo: error: 'git config --get' call failed with return code: %r, stderr: %r",
            retval,
            p.stderr,
        )
    return path
...
class GitCommand:
    ...
    def __init__(
        ...
        event_log = BaseEventLog(env=env, add_init_count=True) if add_event_log else None
                ...
                error_info = json.dumps(
                    {
                        ...
                        "Message": str(e),
                        "ReturnCode": (str(e.git_rc) if e.git_rc is not None else None),
                        "IsError": log_as_error,
                    }
                )
                event_log.ErrorEvent(f"{ERROR_EVENT_LOGGING_PREFIX}:{error_info}")
    ...
    def _RunCommand(
        ...
        stderr = subprocess.STDOUT if merge_output else (subprocess.PIPE if capture_stderr else None)
    ...
    def VerifyCommand(self):
        if self.rc == 0:
            return None
        stdout = "\n".join(self.stdout.split("\n")[:GIT_ERROR_STDOUT_LINES]) if self.stdout else None
        stderr = "\n".join(self.stderr.split("\n")[:GIT_ERROR_STDERR_LINES]) if self.stderr else None
        project = self.project.name if self.project else None
