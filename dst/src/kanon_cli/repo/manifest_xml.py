...
urllib.parse.uses_relative.extend(["ssh", "git", "persistent-https", "sso", "rpc"])
urllib.parse.uses_netloc.extend(["ssh", "git", "persistent-https", "sso", "rpc"])
...
def XmlBool(node, attr, default=None):
    ...
    if s == "":
        ...
        print(
            'warning: manifest: %s="%s": ignoring invalid XML boolean' % (attr, value),
            file=sys.stderr,
        )
        return default
...
class _XmlRemote:
    ...
    def _resolveFetchUrl(self):
        ...
        if manifest_url.find(":") != manifest_url.find("/") - 1:
            fetch_url = urllib.parse.urljoin("gopher://" + manifest_url, fetch_url)
...
class _XmlSubmanifest:
    ...
    def __init__(
        ...
        if self.remote and not self.project:
            raise ManifestParseError(f"Submanifest {name}: must specify project when remote is given.")
        ...
        linkFile = parent.SubmanifestInfoDir(os.path.join(parent.path_prefix, self.relpath), MANIFEST_FILE_NAME)
    ...
    def ToSubmanifestSpec(self):
        ...
        return SubmanifestSpec(self.name, manifestUrl, manifestName, revision, path, groups)
...
"""
    ...
    def ToXml(
        ...
        def output_project(parent, parent_node, p):
            ...
            if peg_rev:
                ...
                if p.upstream and (p.upstream != p.revisionExpr or p.upstream != d.upstreamExpr):
                    e.setAttribute("upstream", p.upstream)
        ...
        def append_children(ret, node):
            for child in node.childNodes:
                if child.nodeType == xml.dom.Node.ELEMENT_NODE:
                    ...
                    element = {attrs.item(i).localName: attrs.item(i).value for i in range(attrs.length)}
    ...
    def IsFromLocalManifest(self, project):
        ...
        return any(x.startswith(LOCAL_MANIFEST_GROUP_PREFIX) for x in project.groups)
    ...
    def SubmanifestInfoDir(self, submanifest_path, object_path=""):
        ...
        """
        if submanifest_path:
            return os.path.join(self.repodir, SUBMANIFEST_DIR, submanifest_path, object_path)
        else:
    ...
    def _Load(self, initial_client=None, submanifest_depth=0):
        if submanifest_depth > MAX_SUBMANIFEST_DEPTH:
            raise ManifestParseError("maximum submanifest depth %d exceeded." % MAX_SUBMANIFEST_DEPTH)
        if not self._loaded:
    ...
    def _ParseManifestXml(
        ...
        for manifest in root.childNodes:
            if manifest.nodeType == manifest.ELEMENT_NODE and manifest.nodeName == "manifest":
                break
        else:
    ...
    def _ParseManifest(self, node_list):
                    ...
                    if remote.name in self._remotes:
                        if remote != self._remotes[remote.name]:
                            raise ManifestParseError(
                                "remote %s already exists with different attributes" % (remote.name)
                            )
                    else:
        ...
        for node in itertools.chain(*node_list):
            if node.nodeName == "default":
                ...
                if self._default is None:
                    ...
                    raise ManifestParseError("duplicate default in %s" % (self.manifestFile))
                ...
                if submanifest:
                    if submanifest.name in self._submanifests:
                        if submanifest != self._submanifests[submanifest.name]:
                            raise ManifestParseError(
                                "submanifest %s already exists with different attributes" % (submanifest.name)
                            )
                    else:
        ...
        for node in itertools.chain(*node_list):
            if node.nodeName == "notice":
                if self._notice is not None:
                    raise ManifestParseError("duplicate notice in %s" % (self.manifestFile))
        ...
        for node in itertools.chain(*node_list):
            if node.nodeName == "manifest-server":
                ...
                if self._manifest_server is not None:
                    raise ManifestParseError("duplicate manifest-server in %s" % (self.manifestFile))
                self._manifest_server = url

        def recursively_add_projects(project):
            projects = self._projects.setdefault(project.name, [])
            if project.relpath is None:
                raise ManifestParseError("missing path for %s in %s" % (project.name, self.manifestFile))
            if project.relpath in self._paths:
                raise ManifestParseError("duplicate path %s in %s" % (project.relpath, self.manifestFile))
            for tree in submanifest_paths:
                if project.relpath.startswith(tree):
                    raise ManifestParseError("project %s conflicts with submanifest path %s" % (project.relpath, tree))
        ...
        for node in itertools.chain(*node_list):
            ...
            if node.nodeName == "extend-project":
                ...
                if name not in self._projects:
                    raise ManifestParseError("extend-project element specifies non-existent project: %s" % name)
                ...
                if dest_path and not path and len(named_projects) > 1:
                    raise ManifestParseError(
                        "extend-project cannot use dest-path when matching multiple projects: %s" % name
                    )
                        ...
                        if base_revision:
                            if p.revisionExpr != base_revision:
                                failed_revision_changes.append(
                                    ...
                                    "%s vs revision %s" % (name, base_revision, p.revisionExpr)
                                )
            ...
            if node.nodeName == "repo-hooks":
                ...
                if repo_hooks_project is not None:
                    raise ManifestParseError("duplicate repo-hooks in %s" % (self.manifestFile))
            ...
            if node.nodeName == "superproject":
                ...
                if self._superproject:
                    raise ManifestParseError("duplicate superproject in %s" % (self.manifestFile))
                remote_name = node.getAttribute("remote")
                if not remote_name:
                    remote = self._default.remote
                else:
                    remote = self._get_remote(node)
                if remote is None:
                    raise ManifestParseError("no remote for superproject %s within %s" % (name, self.manifestFile))
                revision = node.getAttribute("revision") or remote.revision
                if not revision:
                    revision = self._default.revisionExpr
                if not revision:
                    raise ManifestParseError("no revision for superproject %s within %s" % (name, self.manifestFile))
            ...
            if node.nodeName == "remove-project":
                ...
                if not name and not path:
                    raise ManifestParseError("remove-project must have name and/or path")
                ...
                for projname, projects in list(self._projects.items()):
                    for p in projects:
                        ...
                        elif path == p.relpath and (name == projname or not name):
                            if base_revision:
                                if p.revisionExpr != base_revision:
                                    failed_revision_changes.append(
                ...
                if removed_project and removed_project not in self._projects and repo_hooks_project == removed_project:
                    repo_hooks_project = None

                if not removed_project and not XmlBool(node, "optional", False):
                    raise ManifestParseError("remove-project element specifies non-existent project: %s" % node.toxml())

        if failed_revision_changes:
            raise ManifestParseError(
                "revision base check failed, rebase patches and update base revs for: ",
                failed_revision_changes,
            )
        ...
        if repo_hooks_project:
            ...
            try:
                ...
                raise ManifestParseError("project %s not found for repo-hooks" % (repo_hooks_project))

            if len(repo_hooks_projects) != 1:
                raise ManifestParseError("internal error parsing repo-hooks in %s" % (self.manifestFile))
    ...
    def _ParseDefault(self, node):
        ...
        if d.sync_j is not None and d.sync_j <= 0:
            raise ManifestParseError('%s: sync-j must be greater than 0, not "%s"' % (self.manifestFile, d.sync_j))
    ...
    def _ParseSubmanifest(self, node):
        ...
        if path == "":
            ...
            if revision:
                ...
                if msg:
                    raise ManifestInvalidPathError('<submanifest> invalid "revision": %s: %s' % (revision, msg))
            else:
                msg = self._CheckLocalPath(name)
                if msg:
                    raise ManifestInvalidPathError(f'<submanifest> invalid "name": {name}: {msg}')
        else:
            msg = self._CheckLocalPath(path)
            if msg:
                raise ManifestInvalidPathError(f'<submanifest> invalid "path": {path}: {msg}')
    ...
    def _ParseProject(self, node, parent=None, **extra_proj_attrs):
        ...
        if msg:
            raise ManifestInvalidPathError(f'<project> invalid "name": {name}: {msg}')
        if parent:
        ...
        if remote is None:
            remote = self._default.remote
        if remote is None:
            raise ManifestParseError(f"no remote for project {name} within {self.manifestFile}")

        revisionExpr = node.getAttribute("revision") or remote.revision
        if not revisionExpr:
            revisionExpr = self._default.revisionExpr
        if not revisionExpr:
            raise ManifestParseError("no revision for project %s within %s" % (name, self.manifestFile))
        ...
        if not path:
            ...
            if msg:
                raise ManifestInvalidPathError(f'<project> invalid "path": {path}: {msg}')
        ...
        if clone_depth is not None and clone_depth <= 0:
            raise ManifestParseError(
                '%s: clone-depth must be greater than 0, not "%s"' % (self.manifestFile, clone_depth)
            )
        ...
        if parent is None:
            ...
            relpath, worktree, gitdir, objdir = self.GetSubprojectPaths(parent, name, path)
    ...
    def GetProjectsWithName(self, name, all_manifests=False):
        ...
        """
        if all_manifests:
            return list(itertools.chain.from_iterable(x._projects.get(name, []) for x in self.all_manifests))
    ...
    def _CheckLocalPath(path, dir_ok=False, cwd_dot_ok=False, abs_ok=False):
        ...
        * No ".git" or ".repo*" path components.
        ...
        if norm == ".." or (len(norm) >= 3 and norm.startswith("..") and resep.match(norm[0])):
    ...
    def _ValidateFilePaths(cls, element, src, dest):
        ...
        if msg:
            raise ManifestInvalidPathError(f'<{element}> invalid "src": {src}: {msg}')
    ...
    def _get_remote(self, node):
        ...
        if not v:
            raise ManifestParseError(f"remote {name} not defined in {self.manifestFile}")
        return v
    ...
    def _reqatt(self, node, attname):
        ...
        if not v:
            raise ManifestParseError("no %s in <%s> within %s" % (attname, node.nodeName, self.manifestFile))
        return v
