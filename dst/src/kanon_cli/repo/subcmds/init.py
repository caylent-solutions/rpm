...
"""
    ...
    def _SyncManifest(self, opt):
        ...
        clone_filter_for_depth = "blob:none" if (_REPO_ALLOW_SHALLOW == "0") else None
        if not self.manifest.manifestProject.Sync(
            ...
            manifest_name = opt.manifest_name
            raise UpdateManifestError(f"Unable to sync manifest {manifest_name}")
    ...
    def ValidateOptions(self, opt, args):
        ...
        if opt.mirror:
            ...
            if opt.use_superproject is not None:
                self.OptionParser.error("--mirror and --use-superproject cannot be used together.")
        if opt.archive and opt.use_superproject is not None:
            self.OptionParser.error("--archive and --use-superproject cannot be used together.")

        if opt.standalone_manifest and (opt.manifest_branch or opt.manifest_name != "default.xml"):
            self.OptionParser.error("--manifest-branch and --manifest-name cannot be used with --standalone-manifest.")

        if opt.manifest_upstream_branch and opt.manifest_branch is None:
            self.OptionParser.error("--manifest-upstream-branch cannot be used without --manifest-branch.")

        if args:
            if opt.manifest_url:
                self.OptionParser.error(
                    "--manifest-url option and URL argument both specified: only use one to select the manifest URL."
                )
    ...
    def Execute(self, opt, args):
        ...
        if not git_require(min_git_version_soft):
            logger.warning(
                "repo: warning: git-%s+ will soon be required; please upgrade your version of git to maintain support.",
